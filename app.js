...
// ─── 3D MODEL TURNTABLE STAGE (Showcase Sweep with No Mirror Effect & Logo Snap) ──
function initCube() {
  const container = document.getElementById('turntable-container');
  const stage = document.getElementById('stage-rotator');
  const hint = document.getElementById('stage-hint');
  const brandLogos = document.querySelectorAll('.brand-logo, .logo-box, .f-brand');
  ...
  let currentAngleX = 0;
  let targetAngleY = 0;
  let isAutoSweeping = true;
  let isSnapping = false;
  let isDragging = false;
  let startX = 0, startY = 0;
  let sweepTime = 0;

  // Maximum allowed angle so the image NEVER rotates past 60° (no 180° mirror effect!)
  const MAX_ANGLE_Y = 60;
  const SWEEP_AMPLITUDE = 32; // Smooth, elegant 3D perspective sweep (-32° to +32°)
  const SWEEP_SPEED = 0.016;

  // 60FPS Render Loop
  function spinLoop() {
    if (isAutoSweeping && !isDragging) {
      sweepTime += SWEEP_SPEED;
      // Gentle sine-wave turntable oscillation: always facing front, ZERO mirror effect
      currentAngleY = Math.sin(sweepTime) * SWEEP_AMPLITUDE;
      currentAngleX = Math.cos(sweepTime * 0.7) * 3; // subtle natural 3D tilt
    } else if (isSnapping && !isDragging) {
      // Smoothly animate back to front (0deg)
      const diffY = targetAngleY - currentAngleY;
      const diffX = 0 - currentAngleX;

      currentAngleY += diffY * 0.15;
      currentAngleX += diffX * 0.15;
  ...
  // Function: Snap directly to Front View
  function snapToFront(flash = true) {
    ...
    if (hint) {
      hint.innerHTML = '<i data-lucide="sparkles"></i> ✨ Front View Active (Tap to resume 3D motion)';
      if (window.lucide) lucide.createIcons();
    }

    if (flash) {
      stage.classList.remove('stage-highlight');
      void stage.offsetWidth; // retrigger CSS animation
      stage.classList.add('stage-highlight');
    }
  }

  // Logo click functionality: click logo -> immediately show front side & scroll to hero
  ...
  // Tap / Click on the stage container toggles auto-sweep
  function toggleSweep() {
    if (isAutoSweeping) {
      ...
      isSnapping = false;
      if (hint) {
        hint.innerHTML = '<i data-lucide="rotate-3d"></i> 🟢 3D Perspective Active (Tap to freeze front)';
        if (window.lucide) lucide.createIcons();
      }
    }
  }
  ...
  // Drag interaction with strict angle clamp (NEVER mirrors or flips backwards)
  ...
  addEventListener('mousemove', e => {
    ...
    // Strictly clamped between -MAX_ANGLE_Y and +MAX_ANGLE_Y so it never shows mirror backside
    currentAngleY = Math.max(-MAX_ANGLE_Y, Math.min(MAX_ANGLE_Y, currentAngleY + deltaX * 0.4));
    currentAngleX = Math.max(-18, Math.min(18, currentAngleX - deltaY * 0.2));
  ...
  function endDrag() {
    if (isDragging) {
      isDragging = false;
      targetAngleY = Math.max(-SWEEP_AMPLITUDE, Math.min(SWEEP_AMPLITUDE, currentAngleY));
      isSnapping = true;
    }
  }
  ...
  // Global helper if needed
  window.showFrontSide = snapToFront;
}
...
function initForm() {
  ...
  const targetEmail = 'guptashreya2305@gmail.com';
